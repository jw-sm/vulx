...
def parse_nested_list(
    ...
    for idx, item in enumerate(data):
        try:
            parsed = parse_nested(cls, item, f"{context}[{idx}]")
            if parsed:
...
class CVERecord:
    ...
    def validate(self) -> None:
        ...
        if not self.cveMetadata.cveId.startswith("CVE-"):
            raise CVEParseError(
                f"Invalid CVE ID format: {self.cveMetadata.cveId}"
            )
...
def parse_cve_record(json_data: Dict[str, Any]) -> CVERecord:
    ...
    try:
        ...
        cve_metadata = parse_nested(CVEMetadata, metadata_dict, "cveMetadata")
        if not cve_metadata:
        ...
        adp_containers = parse_nested_list(ADPContainer, adp_list, "containers.adp")
...
def parse_cna_container(data: Dict[str, Any]) -> CNAContainer:
    ...
    descriptions = parse_nested_list(Description, descriptions_data, "cna.descriptions")
    ...
    for idx, pt_data in enumerate(problem_types_data):
        ...
        cwe_descriptions = parse_nested_list(
            CWEDescription, 
            cwe_descs_data, 
            f"cna.problemTypes[{idx}].descriptions"
        )
    ...
    for idx, metric_data in enumerate(metrics_data):
        ...
        if cvss_data:
            cvss = parse_nested(CVSSMetric, cvss_data, f"cna.metrics[{idx}].cvssV4_0")
            metrics.append(Metric(cvssV4_0=cvss))
    
    # Parse references
    references_data = data.get("references", [])
    references = parse_nested_list(Reference, references_data, "cna.references")
    ...
    for idx, affected_item in enumerate(affected_data):
        ...
        versions = parse_nested_list(
            Version, 
            versions_data, 
            f"cna.affected[{idx}].versions"
        )
    ...
    provider_metadata = parse_nested(ProviderMetadata, provider_data, "cna.providerMetadata")
    
    # Parse source
    source_data = data.get("source")
    source = parse_nested(Source, source_data, "cna.source")
...
# Example usage demonstrating the parser
if __name__ == "__main__":
    ...
    try:
        ...
        cve = load_cve_from_string(json_string)
